...
namespace Infrastructure.Repositories.GroupJoinRequest
{
        ...
        IGroupJoinRequestReadRepository
    {
        ...
        public Task<List<Domain.GroupJoinRequest.GroupJoinRequest>> GetAllByGroupIdAsync(Guid groupId, CancellationToken cancellationToken)
            => Query()
        ...
        public Task<List<Domain.GroupJoinRequest.GroupJoinRequest>> GetAllByUserIdAsync(Guid userId, CancellationToken cancellationToken)
            => Query()
                .Where(gjr => gjr.UserId == userId)
                .Include(gjr => gjr.Group)
                .ToListAsync(cancellationToken);

        public Task<List<Domain.GroupJoinRequest.GroupJoinRequest>> GetAllPendingByUserIdAsync(Guid userId, CancellationToken cancellationToken)
            => Query()
                .Where(gjr => gjr.UserId == userId
